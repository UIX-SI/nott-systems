...
### 2026-06-06 v12 (풀화면 모달 + 모바일 목업 리뉴얼)
- 크게 보기 모달을 **풀화면 대시보드**로 — PC 콘솔이 실제 풀스크린처럼 크게 렌더
- 모달에 **미리보기 ⇄ 소스코드 슬라이드** 전환 (소스가 뒤에서 슬라이드인)
- **슈퍼앱 모바일 목업 리뉴얼** — One ID · 배터리 · 스마트홈 on/off · 서비스 아이콘 · 오늘의 일정 피드 · 탭바
...
- 우측을 **폰 디바이스 프레임**(모바일 미리보기)으로 변경
- 폰의 **"PC 버전 보기"** → PC(데스크탑) 화면을 브라우저 크롬 두른 **팝업**으로
- 선택 카드 **글로우(그라데이션) 제거** — 과한 번짐 톤다운

### 2026-06-06 v9.1 (UI 폴리시)
- 다운로드 버튼 강조 — 상단 헤더 + **하단 다운로드 바** 양쪽 배치, primary 버튼 확대
- 고객사/모듈 카드: 그림자 + 호버 lift + 선택 시 브랜드색 강조 링
- 카테고리 헤더에 색 액센트 바 + 개수 배지
- 전체 카드 rounded-xl/2xl + shadow-sm로 제품 UI 톤 정리

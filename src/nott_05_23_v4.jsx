...
//
// v3 → v4 변경점 (디자인 절제):
//  - 폰트 사이즈 전반적으로 한~두 단계 축소
//  - font-black → font-semibold/bold (강도 절제)
//  - rounded-[28px/32px] → rounded-2xl/xl (모서리 절제)
//  - 패딩 정돈 (md:p-10 → p-6)
//  - 사이드 패널 폭 420 → 360
//  - 로고 헤더 크기 36 → 26
//  - 미니멀 도구 느낌으로 톤다운
...
export default function ${componentName}() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: theme.background, padding: 32 }}>
      <section style={{ maxWidth: 880, margin: "0 auto", background: "white", borderRadius: 16, padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: theme.primaryColor, fontWeight: 700, fontSize: 16 }}>{theme.logoText}</div>
          <div style={{ fontSize: 11, color: "#94a3b8" }}>powered by nott</div>
        </div>
        <h1 style={{ fontSize: 28, marginTop: 24, fontWeight: 600, letterSpacing: "-0.01em" }}>{theme.serviceName}</h1>
        <p style={{ fontSize: 12, color: theme.primaryColor, marginTop: 4, fontWeight: 600 }}>{theme.serviceTagline}</p>
        <p style={{ fontSize: 14, color: "#64748b", marginTop: 12, lineHeight: 1.6 }}>{theme.serviceDescription}</p>
        ...
        <button onClick={callApi} style={{ marginTop: 16, background: theme.primaryColor, color: "white", border: 0, borderRadius: 999, padding: "10px 18px", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
...
---
Generated on 2026-05-23 by nott (v4).
`;
}
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-5 pb-8 lg:grid-cols-[1fr_340px]">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-3 md:grid-cols-4">
                  {companies.map((company) => (
                    <button
                      ...
                      style={{
                        borderColor: selectedCompany.id === company.id ? company.color : "#e2e8f0",
                        background:  selectedCompany.id === company.id ? company.bg    : "white",
                      }}
                    >
                      <div
                        className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-md text-[11px] font-bold text-white"
        ...
        <aside className="rounded-2xl border border-slate-200 bg-slate-900 p-4 text-white">
          ...
          <div className="mt-3 flex items-center justify-center gap-1.5 text-[10px] text-slate-500">
            <NottSymbol size={14} />
            <span>Easy AI for every SI · v4</span>
          </div>
        </aside>
      </main>
    </div>
...
function PreviewCard({ company, service, compact = false }) {
  return (
    ...
    >
      ...
      <div className="mt-8">
        <div className="text-xs font-semibold" style={{ color: company.color }}>
        ...
        <button
          className="mt-4 block rounded-full px-3 py-1.5 text-xs font-semibold text-white"
          style={{ background: company.color }}
        >
          Start {service.name}
        </button>
